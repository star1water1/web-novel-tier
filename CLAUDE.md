...
- `docs/hollow-feature-audit.md` — **'있으나 마나 한 기능' 감사 47건** (v7.58.4 시점).
  죽은 코드가 아니라 '코드는 도는데 실질 가치가 없거나·반쪽이거나·하드코딩된' 항목들.
  체크리스트 + 도메인별 상세(증거 줄번호·사용자 영향·개선안) + 우선순위 제안이 들어 있다.
  ⚠️ 적대적 검증에서 기각이 0건이라 판정을 그대로 신뢰하면 안 된다 — 고치기 전에 해당 줄을 직접 열어 확인할 것.
