...
- [~] **4b. 제목 검색 — 나머지 플랫폼** (진행 중): 개발환경에선 페이지 fetch 전부 불가(curl·WebFetch 모두 IP/CF 차단, WebSearch도 구조 정보 없음 — 2026-06-21 재확인). → **폰이 직접 캡처하는 '🔧 긁기 진단'(설정>태그, v7.28.38)** 으로 실제 원본 확보 후 fixture 기반 구현.
  - [x] **네이버시리즈** (v7.28.39): `parseNaverSeriesSearch` — 모바일 검색 SSR `<ul id=itemList>`의 `<li class=lst>`를 `/{novel|comic|ebook}/detail.series?productNo=N` 앵커로 추출(제목=img alt 우선·작가·장르·표지·detail URL). `searchTemplate` 더미/푸터폼은 앵커로 자동 제외. `searchNovels`가 리디+시리즈 `Promise.allSettled` 병렬. 픽스처 `naver-series-search.html` + 회귀 14건.
  - [x] **문피아** (v7.28.40): `parseMunpiaSearch` — 모바일 검색 SSR `<ul id=list_ul>`의 `<li onclick=...view_novel(ID)>`에서 제목(배지 span 제거)·작가(첫 구분선 앞)·장르·표지(`//`→https)·detail URL(`?menu=novel&id=`) 추출. 하단 JS 렌더 템플릿/추천영역은 list_ul 슬라이스 + 큰따옴표 `view_novel(숫자)` 앵커로 제외. ⚠️ **2자 미만은 문피아가 `/main`으로 바운스** → `searchMunpia`가 빈 결과로 단축. 픽스처 `munpia-search-hoegwi.html` + 회귀 12건. **URL 도메인은 v7.28.44에서 `mm.munpia.com`으로 최종 교정**(실측: `www`=PC는 모바일 UA에서 `/main` 바운스, `m`=새 React SPA, **`mm`=구버전 SSR(list_ul)**). 로그인 무관 — 도메인 문제였음. v7.28.44부터 검색 SSR의 줄거리·완결·회차·장르를 meta로 동봉해 상세 재긁기 없이 등록.
...
- [x] **4c. 검색 결과 품질·등록 정확화** (v7.28.43, 실사용 피드백): `mergeSearchResults`로 관련도(정확>시작>포함>부분) 정렬 + 플랫폼 균형(라운드로빈, 플랫폼당 12개) → 리디 부분매칭 노이즈 후순위. `parseRidiSearch`가 개별 회차/권(`… N화/N권`) 제외. 노벨피아는 상세가 SPA라 og 부실(제목 사이트명·작가 Devlife) → 검색 API 메타를 후보에 동봉해 상세 재긁기 없이 등록(`openScrapeFromMeta`). 문피아 미노출은 v7.28.44 도메인 교정(`www`→`mm`)으로 해결.
- [x] **4c-2. 등록 값 전수 보강** (v7.28.45~47): ① 태그 적용 버그 수정 — `mapScrapedGenres`가 미매칭 키워드를 대장르가 아닌 **일반 태그**로 분류, `buildScrapeItems`에 `tags` 항목, 4개 ctx(신규/예정/보충/편집) `apply`에 tags 배선. ② 연재 시작/종료연도 — 노벨피아 API(start_date/complete_date/reg_date), 상세 JSON-LD datePublished. ③ 작품 링크·연재처(platforms) 자동 채움 — `meta.url`·`meta.platform`을 항목화(그동안 누락). ④ 연령등급 19금 → 태그(노벨피아 novel_age=19, 문피아 성인배지, 상세 contentRating). 회귀 121/121.
- [x] **4d. 카카오 검색 — 보류 확정** (2026-06-21, 사용자 결정): 검색이 GraphQL POST + 빈 SPA(`__NEXT_DATA__`도 없음) + 봇차단(302)이라 폰 캡처(GET)로 불가. LLM 링크찾기는 content ID가 의미 없는 임의 숫자라 할루시네이션 위험 → 검색 미지원. **카카오 작품은 🔗 링크 붙여넣기로 등록**(상세 `__NEXT_DATA__` 파싱은 정상 — `scraperRefineKakao`). 정 필요하면 PC 개발자도구로 graphql 요청(operationName·query·variables) 1회 캡처 후 `searchKakao`(POST) 구현 가능.
