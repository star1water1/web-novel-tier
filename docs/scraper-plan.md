...
- [~] **4b. 제목 검색 — 나머지 플랫폼** (진단 도구 v7.28.38): 개발환경에선 페이지 fetch 전부 불가(curl·WebFetch 모두 IP/CF 차단, WebSearch도 구조 정보 없음 — 2026-06-21 재확인). → **폰이 직접 캡처하는 '🔧 긁기 진단'(설정>태그)** 추가: 검색 URL 붙여넣기→폰 fetch→원본/요약 클립보드 복사. **사용자가 캡처한 실제 원본으로** `searchNaverSeries`/`searchMunpia`/`searchNovelpia`를 fixture 기반 구현·테스트(리디 패턴). 카카오는 GraphQL(302·인증)이라 여전히 보류.
