...
### ✅ v7.59.4 — 발산(divergence) 감지 (로드맵 T05 · 감사 DAT-2, 코드/구문검증, **온디바이스 미검증**)
설계 5의 '양쪽 분기(충돌) → 1회 선택' 중 **선택 부분**을 구현했다(리비전 보관은 미구현 — 아래 잔여 참고).
- **T05**: push 진입부에서 로컬 `cloud_rev` vs 원격 `rev` 대조. 종전엔 원격 rev만 읽어 `baseRev+1`로 올렸으므로
  A(로컬 5)가 B의 rev 6을 안 받은 채 push하면 B의 스냅샷이 통째로 사라졌다(M4는 인플라이트 변경만 봄).
  **충돌 조건 2개**: `원격 rev > 로컬 기록` **AND** `원격을 마지막에 쓴 기기 ≠ 이 기기`.
  두 번째 조건이 없으면 C2 가드(전환 중 `cloud_rev` 기록 스킵)로 **자기 데이터에도 걸리는 오탐**이 난다.
  `unchanged` 스킵보다 앞에 둔다(원격이 앞선 상태에서 "최신 상태"는 거짓 고지).
- 충돌 종류 구분: `kind:"stale"`(발산) / `kind:"inflight"`(M4). 수동 push는 stale에서 **먼저 받기 / 덮어쓰기 / 취소**,
  덮어쓰기는 2단 확인 후 `force:true`(발산 가드만 우회, M4는 유효). 자동(무음) push는 Alert 금지라 `cloudConflict`
  state → 동기화 카드 배지(원격 rev·시각 / 이 기기가 아는 rev / '자동 백업을 멈췄다' / 다음 행동)로 남긴다.
- 배지 해제 3지점(push 성공·pull 성공·무변경 확인) + 슬롯 전환 M1 재로드에서도 초기화(슬롯별 값).
- **잔여**: 직전 리비전 보관은 여전히 없다(push 성공 시 옛 스냅샷 삭제 유지) → 로드맵 T31에서 표지 자산화와 함께 검토.
  그래서 '덮어쓰기'는 복구 불가이며, 확인 문구에 그 사실을 명시했다.
...
- [ ] 충돌(양쪽 변경) 선택 — A·B 각각 편집 후 B가 push → A가 push 시 '먼저 받기/덮어쓰기/취소'가 뜨는지, 자동 push는 무음 중단 + 배지인지 (v7.59.4 구현, 온디바이스 미검증). 직전 리비전 복구는 미구현(T31).
