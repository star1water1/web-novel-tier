...
| [x] | `ANA-1` | 🟡 중간 | ✅ 수상 확률·자동 추천이 티어 가중 200점에 독점돼 티어 배지의 재표현일 뿐이고, 같은 티어 안에서는 사용자의 manual_order를 완결/재독/태그 보너스가 눌러버린다 **✅ v7.59.16 수정(hybrid/manual은 pre-select 재정렬을 생략하고 pool=compareNovels 순서 그대로 — 실측 종전 A2·A3 / 수정 A1·A2. 확률 클램프 1~99 제거 + `winProbText`로 `<1%`/`>99%`/정수 % 통일(열 합계 102.97%→100.00%). 점수식·티어 가중은 무수정 — 티어 횡단 역전 방지는 v7.10.0 의도 동작)** |
| [x] | `ANA-2` | 🟡 중간 | 🟨 취향 분석 최상단 '핵심 취향' 문장이 평점·티어를 전혀 안 쓰고 보유 개수 1위만 읊는다 **✅ v7.59.17 수정(빈도 1위 + 만족도 1위(adjRating, 표본 3작↑) 병기 — 다르면 두 축을 나란히. 공유 배열은 사본 정렬이라 칩·차트 순서 불변. 표본 게이트는 `reliableTotal < 10` → 보류 문구)** |
| [x] | `ANA-3` | 🟡 중간 | ✅ '취향 스펙트럼'의 선호 배지가 평점과 무관한 태그 분포 평균이고, 작품 2개면 표시된다 **✅ v7.59.18 수정(접힌 배지에 `· N작` 병기 + 확장 시 '전체 분포'/'특히 높게 평가한 N작'을 나란히 — 고평가작 0작이면 폴백값 대신 비교 안 했음을 명시. 게이트 2는 `SPECTRUM_MIN_NOVELS` 상수로만 승격, 상향은 T21의 빈 축 사유 표시와 동시에)** |
| [x] | `ANA-4` | 🟡 중간 | 🟨 스펙트럼 6축의 태그 목록이 하드코딩이고 편집 UI가 없으며, 태그 이름을 바꾸면 축이 조용히 비어버린다 **✅ v7.59.19 수정(renameTagGlobally가 spectrumGroups.tags도 이전 — 레지스트리 필드 중 유일한 누락이었다. 알림 문구에 '스펙트럼' 추가. 병합으로 태그가 1개 이하로 줄면 분석이 사유로 돌려 '중간 성향'만 내는 무의미한 축을 안 그린다). 축 편집 UI는 T30 🅓** |
| [x] | `ANA-5` | 🟡 중간 | ✅ 🔍 웹 추천의 '취향순' 정렬이 ELO 승률에만 의존해, hybrid/manual 모드에서는 모든 후보 점수가 0이 되어 정렬이 무동작이다 **✅ v7.59.12 수정(승률이 없으면 buildTasteKeywordPool 재사용해 서재 기반 시드 — prefScore가 mode-aware. 근거 표기 `webRecoTasteBasis` 안내줄 + 칩 캡션)** |
| [x] | `ANA-8` | 🟡 중간 | ✅ '선호 길이' 판정이 100화/400화 하드코딩 버킷 + 최소 표본 0이라, 작품 1개짜리 버킷이 헤드라인 문장을 결정한다 **✅ v7.59.17 수정(버킷 표본 하한 5 + 비교 가능 버킷 2개 미만이면 unknown → 회차 미입력·1작 확정·웹툰 쏠림이 전부 판단 보류로. '다양한 길이' 대신 보류 문구 — 모르는 것을 '다양하다'로 바꾸지 않는다). 경계 100/400 자체는 미변경 — `getEpBucket`이 같은 경계로 `episode_length` 패턴을 DB에 영속하므로 마이그레이션 선행 필요 → T33** |
...
| [x] | `HC-2` | 🟡 중간 | 🟨 스펙트럼 6종은 태그 목록까지 완전 하드코딩 — 형제 기능인 '좌표계'는 사용자가 추가·편집·삭제까지 되는데도 **✅ v7.59.19 부분 수정(축이 표시되지 않을 때 사유를 밝힘 — `spectrumSkipped`로 '태그 부족'/'표본 부족'을 구분, 표시할 축이 0개여도 섹션을 열어 설명. rename 이전으로 사용자 태그 체계를 따라간다). 축 추가·편집 UI 자체는 T30 🅓** |
...
| [x] | `HYB-3` | 🟡 중간 | 🟨 수문장 'AI 제안' 모달이 방향을 제안하지 않고, 거절 수단도 없어 배지가 영구히 남는다 **✅ v7.59.9 수정(세션 suspicion_type 방향 집계 → 우세 방향 '· 추천' 강조 + 근거 줄, '이 제안 무시' + gatekeeper_dismissed 기준선 — blocker_id 이력은 보존)** |
| [x] | `HYB-5` | 🟡 중간 | ✅ 세션당 최대 11회 비교·후보 풀 64·수문장 임계 3이 전부 상수 — 설정 UI에는 '빈도'만 있고 '길이'가 없다 **✅ v7.59.10 수정(suspicionConfig에 gallopMax 5~8 · gatekeeperThreshold 2~5 추가, 풀은 2^(gallopMax−1) 파생 — 진행 중 세션은 시작 시점 스냅샷)** |
...
| [x] | `REC-1` | 🟡 중간 | 🟨 🔍 넷상 추천 '정렬' 칩 4개가 화면 순서에 전혀 반영되지 않는다 — loadWebReco가 항상 taste_score DESC로 덮어씀 **✅ v7.59.11 수정(ORDER BY 마지막 키를 rowid ASC로 — 선정 순서가 곧 표시 순서)** |
| [x] | `REC-2` | 🟡 중간 | ✅ 🔍 '인기순'/'숨은작' 정렬은 노벨피아 작품에만 popularity가 존재해 사실상 '노벨피아 골라내기'로 동작 **✅ v7.59.11 수정(보유/미보유 분리 정렬 + 보강·INSERT의 `|| 0` 제거(NULL 보존) + 칩 캡션으로 한계 고지)** |
| [x] | `REC-3` | 🟡 중간 | 🟨 밴 키워드가 '취향 키워드 풀'에는 적용되지 않아, 기본 설정에서 검색어 3개 중 2개가 밴을 우회한다 **✅ v7.59.13 수정(buildTasteKeywordPool의 add 한 곳에 banned 가드 → 패턴·폴백 경로 동시 커버 + AI 생성 키워드 append에도 가드. 시그니처 무변경이라 미리보기와 실제가 자동 일치)** |
| [x] | `REC-5` | 🟡 중간 | 🟨 'AI 키워드 생성' 토글: 매번 LLM을 호출하지만 생성된 2개가 실제 검색어로 뽑힐 확률은 1% 안팎 **✅ v7.59.15 수정(일 1회 생성 캐시 — 같은 날 재시도는 저장분 재사용, 키/공급자/개수 변경 시 지문으로 자동 무효화. 가중을 풀 총합 대비 지분(0.10)으로 동적화 — 고정 1.5는 400행 풀에서 0.7%까지 희석됐다. 토글 힌트에 호출 빈도·harvest 의존 명시)** |
| [x] | `REC-6` | 🟡 중간 | 🟨 '취향 ↔ 탐험' 슬라이더가 취향 점수엔 관여하지 않고 하드코딩된 8개 카테고리 버킷만 재조정하며, 배정이 UI 설명과 반대다 **✅ v7.59.14 수정(planned_unread를 탐험 축으로 재배정 — 2차 재검증대로 불일치는 이 1건. 기본 70 편향은 '50이 중립' 명문화로 해소, taste.score 계수화는 분류 임계 오염 위험으로 제외)** |
| [x] | `REC-8` | 🟡 중간 | ✅ 취향 승률 백분율이 표본 5건에서도 그대로 노출되고, 코드가 내세운 sample_size>=3 게이트는 win_rate가 NULL이라 아무 의미가 없다 **✅ v7.59.13 수정(refreshPatternStats 문턱 5→3으로 소비 게이트와 일치 — NULL 행이 폴백 25점까지 막던 역전 해소. 과대평가는 _confGate가 자동 보수화, is_notable(n≥15)은 무영향. 칩에 표본 병기 `80% · 3전`)** |
...
| [x] | `ANA-7` | ⚪ 낮음 | 🟨 '상위 티어 집중도'가 표본 2~3에서 '상위 100% (N=2)'를 순위로 내보내고, 계산해 둔 윌슨 신뢰구간은 화면에 안 쓴다 **✅ v7.59.18 수정(정렬 키를 `topTierCI.lower`로 — 실측 N=2·100%가 1위→5위, N=20·80%가 1위. 하한 동률이면 표본 큰 쪽. 정렬 근거 캡션 + entropy 목록에도 N 병기. 임계 상향은 불요 — 윌슨이 자동 강등)** |
...
| [x] | `HC-3` | ⚪ 낮음 | 🟨 하이브리드 모드의 핵심 UX인 '검증 질문 최대 11회'와 '수문장 기준 3'이 설정 불가 — 같은 화면의 자동매칭/의심도는 임계값을 전부 노출 **✅ v7.59.10 수정(HYB-5와 동일 — '한 작품 점검 길이'·'AI 제안 임계' 스테퍼)** |
| [x] | `HC-4` | ⚪ 낮음 | 🟨 의심도 설정 8개 필드 중 decay·cap만 스테퍼가 없어, 프리셋을 누를 때 몰래 바뀌고 되돌릴 방법이 없다 **✅ v7.59.10 수정(decay 스테퍼 0.5~0.95 + 읽는 쪽 하한 가드. cap은 전 프리셋 20 동일이라 2차 재검증에서 기각 — 스테퍼 불요)** |
...
| [x] | `HYB-4` | ⚪ 낮음 | 🟨 수문장 모달의 '한 단계 아래' 강등이 실제로는 하위 티어 최하위로 처박는다 **✅ v7.59.9 수정(강등만 MIN-50 삽입 + rebalanceTierOrder → 하위 티어 맨 위, 확인 다이얼로그에 '지금 → 이동 후' 표기)** |
...
| [x] | `REC-7` | ⚪ 낮음 | 🟨 넷상 추천작이 자정에 통째로 삭제되며 보존 기간을 바꿀 설정이 없다 (WEB_RECO_TTL_DAYS 상수는 참조조차 안 됨) **✅ v7.59.14 수정(상수를 컷오프에 달력일 뺄셈으로 배선 — TTL=1이면 종전과 동일. 설정 노출은 2차 재검증대로 기각: fetch마다 전량 교체라 '며칠 보관'을 못 지킨다)** |
